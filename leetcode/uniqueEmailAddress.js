...
const uniqueEmailAddress = (emails) => {
    ...
    for (const email of emails) {
        ...
        let rest = email.slice(email.indexOf('@'));
        if (local.includes("+")) {
            local = local.slice(0, local.indexOf('+')); // remove all the test after '+'.
        }
        local = local.replace(/\./g, ""); // replace all the '.' with ''.
        map.set(local + rest);
    }
    console.log(map);
    return map.size;
...
console.log(uniqueEmailAddress(test1));